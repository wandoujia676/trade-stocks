...
### 回测实测结果（2026-05-07 实测）

**测试条件**：20 只沪深 300 权重股 × 2024 全年 × 止损 8% / 止盈 20%

| 配置 | 总收益率 | 胜率 | 交易次数 | 最大回撤 | 夏普比率 |
|------|----------|------|----------|----------|----------|
| **Baseline (v8.x 5维度)** | **242.9%** | **58.8%** | **34 笔** | **11.9%** | **0.79** |
| **v9.0 完整版 (8维度+观察池)** | 0% | - | 0 笔 | 0% | 0 |

**Baseline 验证通过**：回测框架可用，5 维度评分体系能筛出有盈利能力的股票。

**v9.0 完整版受限**，原因如下：

### 实测发现的限制

1. **基本面/资金面/催化剂数据依赖 Tushare 高级权限**
   - 免费账户无法拉取 `fina_indicator` / `moneyflow` 等接口
   - 回测中这 3 个维度全部被跳过，v9.0 退化为「5 维度 + 观察池」
   - **解决方案**：充值 Tushare 积分（2000+），或等待 AKShare 补充免费的基本面/资金面接口

2. **观察池右侧确认条件过严**
   - 当前条件：**实体阳线（close > open）+ 量比 > 1.5**，T+1~T+3 内任一天满足
   - 2024 全年实测：20 只 × 13 个月 = 260 次选股，**全部未通过右侧确认**
   - 原因分析：沪深 300 权重股波动小，很难同时满足「收阳」+「量比 1.5 倍」
   - **建议调整（v9.1）**：
     - 方案 A：放宽量比阈值 1.5 → 1.2
     - 方案 B：改为「实体阳线 OR 量比 > 1.5」（满足任一即可）
     - 方案 C：根据股票市值分级确认条件（大盘股用宽松条件，小盘股用严格条件）

3. **回测数据 lookahead bias**
   - 基本面/资金面/催化剂接口只能拿最新数据，回测中用最新数据近似历史
   - 即使有 Tushare 高级权限，回测结果也是上限参考，非真实表现

4. **AKShare 的 period 自动切换 bug（已修复）**
   - 原代码根据时间跨度自动将 daily → weekly/monthly，导致数据行数不足
   - 已在 `data_fetcher.py` 中修复（v9.0 发布后的 hotfix）

### 当前限制
1. 观察池容量无上限：理论上可能积累大量未确认股票（实际使用中不太可能）
2. 右侧确认窗口固定 3 天：未来可考虑根据市场环境动态调整

### 未来计划（v9.1+）
- [ ] **放宽观察池确认条件**（量比 1.5 → 1.2 或改为 OR 逻辑）
- [ ] AKShare 补充免费的基本面/资金面接口（替代 Tushare 高级版）
